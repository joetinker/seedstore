...
// Show
bool showLine(char *line1, char *line2, byte &wordNum) {
  cursorOn = false;
  lcd.setCursor(0,0); lcd.print(line1); 
...
if (setupMode) 
  switch(setupItem) {
    ...
    break;
    case 15: 
      if (saveWordsQ(selectedSeed)) setupItem = 20; // exit 
    break;  
    case 16: 
      if (yesNo("Change descr. ?",yes)) 
        if (yes) {setupItem = 19; } 
        else setupItem = 20; // exit setup
      break;      
