...
static line_t line = {0,true,false,false,""};  // cursor, changed, numeric, hidden, value
static line_t PIN = {0,true,true,true,""};
...
bool delayedAcess() {
  ...
  if (runtime%10 == countDown%10) {countDown--; 
    lcd.setCursor(0,0); lcd.print("Delayed access"); lcd.setCursor(0,1); lcd.print(countDown/86400); lcd.print("D "); 
...
void setup() {
  ...
  if (get_security() & 3) lcd.print("UNLOCK"); 
...
if (setupMode) 
  switch(setupItem) {
    ...
    case 10: if (yesNo("Change delay?",yes)) 
      if (yes) {setupItem = 11; loadDelay(selectedSeed); itoa(delay10days*10,PIN.txt,10); PIN.cursorPos=strlen(PIN.txt); } 
    ...
    case 12: 
      if (yesNo("Change words?",yes)) 
        if (yes) {setupItem = 13; } else {setupItem = 1; menuItem = 1; setupMode = false; }; 
      break;
    case 13: 
      if (showWords(selectedSeed, wordNum)) {//memcpy(line.txt,sword[wordNum],wordLen); line.txt[wordLen]='\0';
          getWord(sword[wordNum]).toCharArray(line.txt,wordLen+1); line.end(); setupItem = 14; };
      if (lcd_key == btnLEFT) {setupItem = 15; lcd.clear(); }; break;  
    case 14: 
      if (editSecureWord(selectedSeed, wordNum)) setupItem = 13; break;
    case 15: 
  ...
  switch (menuItem) {
    ...
    case 31: // restore access when is PIN loss
      ...
      if (lcd_key == btnNONE && lastKeyPress == btnLEFT) {
          lastKeyPress = btnNONE; menuItem = 1; lcd.clear(); /* countDown = oneDayDelay*90; */ };   
    break;
